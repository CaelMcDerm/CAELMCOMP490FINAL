...
So I had to pivot. Instead of a checklist, the Small widget became a Progress Gauge, a single completion metric like ``3/12 Servings.'' You get a status check on a crowded Home Screen, and the Medium and Large widgets pick up the slack for per-category interaction. Keeping the Home Screen widget view-only rather than interactive lined up with what the formative survey showed: the 87.5 percent of people who picked Status-Only over Quick-Add. The ring itself uses a red/yellow/green ramp tied to completion percentage. I asked participants about this during the formative survey and they said the mapping felt natural, since a lot of other apps already use red/orange/green to mean ``bad / meh / good.'' The color ramp piggybacks on a convention users already know, so the ring reads at a glance without needing the numeric label.
...
\begin{figure}[t]
    ...
    \caption{The Apple Watch complication in two configurations. Left: the view-only \texttt{.accessoryCircular} complication at 1/12 completion (early day, red), abstracting the full checklist into a single glanceable metric. This is the version tested in the user study. Right: a design mockup of the Smart Stack variant, which would surface two predicted quick-add pills (``+ Greens,'' ``+ Beans'') alongside the ring. This mockup illustrates the time- and location-aware prediction described in Section 8.2, and was not part of the tested build.}
...
\noindent\textbf{Access and Inequity:} The whole intervention only runs on Apple hardware, and specifically on iOS 17 or later. That means it's only usable by people who can afford a relatively new iPhone, and more narrowly still for the Smart Stack and Watch complication features that require an Apple Watch. The populations most affected by diet-linked chronic disease in the US (particularly low-income communities and communities of color) are the populations least likely to own this hardware, which is the digital-divide concern that Veinot et al.\ call the ``intervention-generated inequality'' problem in health technology: interventions designed on and for well-resourced users can actually widen health gaps rather than close them.\cite{VeinotGoodIntentions} A public-health framing of this project has to acknowledge that the people who need easier dietary tracking the most are not the people this version of the tool reaches. An Android or web version would help, but the deeper issue is that food access and cultural fit matter at least as much as interface friction, and an interface fix can't solve either one.

\noindent\textbf{Whose ``Healthy''?:} The Daily Dozen isn't a neutral nutritional framework. It encodes a specific set of assumptions (a whole-food, plant-based diet, Greger's twelve categories) that don't map cleanly onto every cultural or household food tradition. A user whose staple diet centers on foods outside those twelve categories will see an interface that treats their normal eating as a deficit to be corrected. This isn't unique to the Daily Dozen (almost every dietary tracker builds in some notion of ``correct'' eating), but glanceable widgets make the value judgment more present in the user's day. The Home Screen progress ring is, effectively, a persistent reminder that Greger's categories are the measure. That needs to be on the table as a design choice, not a neutral technical affordance.
...
\noindent\textbf{Psychological Impact:} Gamifying diet through always-on widgets could push vulnerable users toward orthorexia or obsessive tracking.\cite{HayesSirensCall} Seeing incomplete dietary tasks sitting on your screen all day could read as pressure rather than support. The color ramp is the most obvious place this concern shows up: the progress ring goes from red at low completion to green at high completion, which some users could read as the widget flagging them as ``failing'' early in the day. When I asked about this in the formative survey, participants actually preferred the red/yellow/green mapping because it matched conventions from other apps and gave them a quick-read sense of where they stood. I also kept the design away from heavier punitive signals like alert badges, vibration, or notification pings. The widget can only be seen when the user chooses to look at it, which limits how hard it can nag.
...
The system is built in Swift 5 with SwiftUI. There are three compile targets in the Xcode project: the main app, a widget extension bundle (which hosts the Home Screen, Lock Screen, and Watch views), and the App Intent that handles background logging. The three targets can't share memory directly, so they all communicate through App Groups. Figure \ref{fig:architecture} shows the data flow.

\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{architecture_diagram.pdf}
    \caption{System architecture. The main app, widget extension, and App Intent are separate compile targets that can't share memory, so they all communicate by reading and writing to an App Groups shared container. A tap on the widget fires the App Intent, which writes a new log to the shared container and then calls \texttt{reloadAllTimelines()} (dashed line) to push the widget to re-render with the updated state.}
    \label{fig:architecture}
\end{figure}

\noindent\textbf{Main App Target (\texttt{DailyDozen}):} Hosts the full SwiftUI UI and the \texttt{DailyDozensManager} class, which conforms to \texttt{ObservableObject} so SwiftUI views inside the app can subscribe to state changes. The manager is the single source of truth for the twelve Daily Dozen categories and the per-day serving counters. When the user opens the main app and modifies state, the manager serializes that state to the shared \texttt{UserDefaults} (via App Groups) so the widget extension can read it.

\noindent\textbf{Widget Extension (\texttt{DailyDozenWidgetExtension}):} A separate bundle that hosts all three widget surfaces (Home Screen Small/Medium/Large, Lock Screen \texttt{.accessoryRectangular} and \texttt{.accessoryCircular}, and the Apple Watch complication). They're grouped in one extension because they share the same \texttt{TimelineProvider} and \texttt{SimpleEntry} data format; only the view code differs per surface. The extension is headless: it can't keep state of its own between timeline refreshes. It reads the current Daily Dozen state from App Groups on every refresh.

\noindent\textbf{App Intent (\texttt{LogServingIntent}):} A small, isolated target that handles the ``write'' side of the system. When the user taps an interactive element on a widget, iOS fires the intent's \texttt{perform()} function in a background process. That function reads the current state from App Groups, bumps the relevant serving counter, writes the new state back, and then calls \texttt{WidgetCenter.shared.\linebreak reloadAllTimelines()}. The \texttt{reloadAllTimelines} call is what makes the widget re-render with the updated progress ring without the main app needing to launch.

The reason for this three-target split is that widget extensions and App Intents both have to be separate compile targets from the main app in order to run in the background. Putting \texttt{DailyDozensManager} inside the main app and having everything else read from App Groups keeps state ownership centralized in one place, even though the code actually runs in three separate processes.

To extend the project, most changes will fall into one of three buckets: adding a new surface (a macOS widget, for example) means adding a new view to the widget extension and writing a new \texttt{TimelineProvider} configuration; changing what gets logged or how it's logged means editing \texttt{DailyDozensManager} in the main app and updating the serialization format the App Groups container uses; and changing the one-tap logging behavior means editing the \texttt{LogServingIntent}'s \texttt{perform()} function.
